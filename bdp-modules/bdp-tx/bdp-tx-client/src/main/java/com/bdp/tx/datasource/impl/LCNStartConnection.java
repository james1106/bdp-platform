...
/**
 * 事务发起方获取的java.sql.Connection的代理类，很重要，分布式实现的实现奇迹发生的地方，<br/>
 * LCNConnection有两个实现，分别是LCNStartConnection，LCNDBConnection，<br/>
 * 这两个实现的区别在于LCNStartConnection要处理补偿事务
 */
public class LCNStartConnection extends AbstractTransactionThread implements LCNConnection {
	...
	public LCNStartConnection(Connection connection, ICallClose<ILCNResource<?>> subNowCount) {
		...
		if (TxCompensateLocal.current() != null) {// 表示当前为补偿事务
			isCompensate = true;
	...
	public void transaction() throws SQLException {
		...
		try {
			...
			logger.info(" lcn start transaction over, res -> groupId:" + getGroupId() + " and  state is "
					+ (rs == 1 ? "commit" : "rollback"));
		} catch (SQLException e) {
			rollbackConnection();
			// 设置本地事务提交是不成功，将进行事务补偿
